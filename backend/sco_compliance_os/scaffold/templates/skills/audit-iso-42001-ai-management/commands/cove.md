...
## Quando
Come quality assurance finale di **ogni output** prodotto dalla skill audit-iso-42001-ai-management prima della consegna all'utente. Applicato SEMPRE a: `/gap`, `/aiia`, `/fria`, `/soa`, `/roadmap`, `/audit`, `/nc`. Opzionale ma consigliato per output rapidi (`/rischio-regolatorio`, `/ruoli`, `/autorita`).
...
Applicare i principi di `voce-consulenziale-formale` e `disaiizzatore-testi-tecnici-normativi`:
...
- (integrazione con skill `voce-consulenziale-formale` e `disaiizzatore-testi-tecnici-normativi` quando disponibili)
