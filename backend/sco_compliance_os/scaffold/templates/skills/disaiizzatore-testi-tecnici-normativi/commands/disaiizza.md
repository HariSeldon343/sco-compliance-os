...
| `--lingua-stile` | `consulenziale formale` \| `neutro` | `consulenziale formale` | Stile-target della riscrittura |
...
4. **Test stile-target**: il passaggio è già coerente con `consulenziale formale-voice`?
   - Sì → escludi
   - No → procedi
...
3. Aggiungi (se opportuno) il riferimento normativo puntuale che lo stile consulenziale formale richiederebbe.
...
secondo il catalogo. Il registro è coerente con lo stile-target consulenziale formale.
```
