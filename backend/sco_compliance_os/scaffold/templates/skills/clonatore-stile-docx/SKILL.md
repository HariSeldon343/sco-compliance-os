...
# clonatore-stile-docx — Clonazione fedele dello stile editoriale di un documento Word
...
Per la redazione del contenuto del nuovo documento, attiva la skill `voce-consulenziale-formale` se il documento è una nota tecnica, relazione, report di assessment, parere, o comunicazione professionale. La skill `voce-consulenziale-formale` gestisce il **tono di scrittura**, mentre `clonatore-stile-docx` gestisce esclusivamente la **veste grafica**.
