...
async def main() -> None:
    ...
    if license_path.exists():
        ...
        print(f"    EXISTS — email={data.get('email')} validated={data.get('validation_result', {}).get('is_valid')}")
    else:
        ...
        print(f"    CREATA fake active-license.json")
    ...
    try:
        ...
        async for ev in runner.stream("Rispondi solo 'pong' e basta."):
            ...
            if ev.kind == "text_delta":
                ...
                print(f"    [{ev.seq}] DONE: stop_reason={ev.data.get('stop_reason')} usage={ev.data.get('usage')}")
        ...
        if "pong" in full_text.lower():
            print(f"    PASS — agent SDK reale ha risposto correttamente")
        elif "stub" in full_text.lower() or "sto rispondendo" in full_text.lower():
            print(f"    FAIL — sta girando lo STUB OLD (testo hardcoded 'Ciao, sono lo stub...')")
            print(f"    --> Backend uvicorn ha caricato versione PRE-commit 6fd6c01")
        else:
